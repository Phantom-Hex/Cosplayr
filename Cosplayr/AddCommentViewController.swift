...
class AddCommentViewController: UIViewController, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    @IBAction func saveComment(_ sender: CustomizableButton){
        ...
        if commentSwitch.isOn {
            ...
            if let image = self.commentImageView.image {
                ...
                if let imageData = UIImageJPEGRepresentation(image, CGFloat(0.35)){
                    ...
                    self.netService.uploadImageToFirebase(postId: commentId, imageData: imageData, completion: { (url) in
                        let comment = Comment(commentId: commentId, userId: Auth.auth().currentUser!.uid, commentText: commentText, postId: self.postId, commentImageURL: String(describing:url), commentDate: commentDate, type: "IMAGE")
            ...
            let commentId = NSUUID().uuidString
            let commentDate = NSDate().timeIntervalSince1970 as NSNumber
            let comment = Comment(commentId: commentId, userId: Auth.auth().currentUser!.uid, commentText: commentText, postId: self.postId, commentImageURL: "", commentDate: commentDate, type: "TEXT")
